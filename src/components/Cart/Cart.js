...
const Cart = () => {
  ...
  const productItemCart = items.map((product) => {
    return (
      <div
        id={product.id}
        key={product.id}
        className="flex justify-between items-center bg-white rounded-xl p-6 border-solid border border-gray-600 shadow-lg shadow-blue-800/40 "
      >
        <h3>{product.name}</h3>
        <p>{product.price}</p>
  ...
  return (
    <form
      ...
      className="w-2/3 flex flex-col p-3 gap-5 mx-auto"
    >
      <div className="flex gap-3 ">
        ...
        <div className="flex flex-col gap-2 w-3/5 border-solid border-2 border-gray-600 rounded-xl shadow-xl shadow-gray-800/40  p-5">
          {productItemCart}
        </div>
      </div>
